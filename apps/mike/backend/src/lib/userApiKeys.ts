...
import { isOllamaConfigured } from "./llm";
...
export type ApiKeyStatus = Record<Provider, boolean> & {
    sources: Record<Provider, ApiKeySource>;
};
...
const STATUS_PROVIDERS: Provider[] = ["claude", "gemini", "openai", "ollama"];
...
export async function getUserApiKeyStatus(
    ...
    const status: ApiKeyStatus = {
        ...
        sources: {
            ...
            gemini: null,
            openai: null,
            ollama: isOllamaConfigured() ? "env" : null,
        },
    };
...
export async function getUserApiKeys(
    ...
    const apiKeys: UserApiKeys = {
        ...
        ollama: isOllamaConfigured() ? "local" : null,
    };

...
create table if not exists public.signature_requests (
  ...
  provider text not null default 'opensign'
    check (provider = any (array['opensign'::text])),
...
create index if not exists signature_recipients_request_id_idx
  on public.signature_recipients(signature_request_id, signing_order);

create index if not exists signature_recipients_provider_id_idx
  on public.signature_recipients(provider_recipient_id)
...
create index if not exists signature_events_request_id_idx
  on public.signature_events(signature_request_id, received_at desc);

create unique index if not exists signature_events_provider_event_uidx
  on public.signature_events(provider, provider_event_id)
...
revoke all on public.signature_recipients from anon, authenticated;

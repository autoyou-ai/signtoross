...
import { SignatureRequestModal } from "@/app/components/shared/SignatureRequestModal";
import { SignatureStatusChip } from "@/app/components/shared/SignatureStatusChip";
...
export function ProjectPage({ projectId, initialTab = "documents" }: Props) {
    ...
    const [signatureDoc, setSignatureDoc] = useState<MikeDocument | null>(null);
    ...
    async function submitSignatureRequest(
        payload: Parameters<typeof createSignatureRequest>[1],
    ) {
        if (!signatureDoc) return;
        const request = await createSignatureRequest(signatureDoc.id, payload);
        const summary: MikeSignatureRequestSummary = {
            id: request.id,
                ...
                ? {
                      ...
                      documents: (prev.documents ?? []).map((doc) =>
                          doc.id === signatureDoc.id
                              ? {
                                    ...doc,
                                    latest_signature_request: summary,
                                }
                              : doc,
                      ),
                  }
    ...
    async function downloadSignedRequest(requestId: string) {
        const { url, filename } = await getSignatureRequestSignedUrl(requestId);
        const a = document.createElement("a");
        a.href = url;
        a.download = filename;
        a.click();
    ...
    async function downloadLatestSignedDoc(doc: MikeDocument) {
        const request = doc.latest_signature_request;
        if (!request?.id || request.status !== "completed") return;
        await downloadSignedRequest(request.id);
    }
                                    ...
                                    >
                                        {menuDoc ? (
                                            <RowActionMenuItems
                                                ...
                                                onDownloadSigned={
                                                    menuDoc.latest_signature_request
                                                        ?.status ===
                                                        "completed" &&
                                                    menuDoc.latest_signature_request
                                                        .signed_pdf_path

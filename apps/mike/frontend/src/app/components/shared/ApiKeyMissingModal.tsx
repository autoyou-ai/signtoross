...
export function ApiKeyMissingModal({ open, onClose, provider, message }: Props) {
    ...
    const title = isLocalModel ? "Local model unavailable" : "API key required";
    const body =
        message ??
        (isLocalModel
            ? "Ollama is not enabled for this backend. Set AI_PROVIDER=ollama or OLLAMA_ENABLED=true, set OLLAMA_MODEL in backend/.env, then restart the backend."

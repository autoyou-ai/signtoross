...
export type ModelProvider = ApiKeyProvider | "ollama";
...
export type ApiKeyStatus = Record<ModelProvider, boolean> & {
    sources?: Partial<Record<ModelProvider, ApiKeySource>>;
};
...
): Promise<MikeSignatureRequest> {
    return apiRequest<MikeSignatureRequest>(
...
export async function listSignatureRequests(
    ...
    const response = await apiRequest<{ requests: MikeSignatureRequest[] }>(
        `/single-documents/${documentId}/signature-requests`,
    );
...
export async function getSignatureRequest(
    ...
    return apiRequest<MikeSignatureRequest>(`/signature-requests/${requestId}`);
}
...
export async function getSignatureRequestSignedUrl(
    requestId: string,
